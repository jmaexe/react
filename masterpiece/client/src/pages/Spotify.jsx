...
const Spotify = () => {
  ...
  useEffect(() => {
    ...
    const fetchTopArtistsTracks = async () => {
      setLoading(true)
      try {
        const response = await axios.get('https://api.spotify.com/v1/me/top/tracks?time_range=short_term&limit=' + infoTracks.quantity, { headers: { Authorization: `Bearer ${token}` } })
    ...
    <div className='flex justify-center flex-col gap-4'>
      {token ?
        <>
          {!error && <>
            <div className='flex justify-center flex-wrap'>
              <div className={`dropdown dropdown-${window.innerWidth < 468 ? 'bottom' : 'left'}`}>
              ...
              <div>
                <button className='btn' onClick={logoutFromSpotify}>Logout from Spotify</button>
              </div>
            </div>
            :
        ...
        <div className='flex items-center justify-center w-full'>
          <a href={`https://accounts.spotify.com/authorize?client_id=${process.env.CLIENT_ID}&client_secret=${process.env.CLIENT_SECRET}&response_type=token&redirect_uri=${process.env.REDIRECT_URI}&scope=${process.env.SCOPE}&show_dialog=true`} className='btn btn-link text-2xl'>Authorize to get the songs</a>
        </div>
      }
